"""Exact Storage YPrim decoder from target-free datakit metadata."""
...
def decode_storage_metadata(
    ...
    if not all(hasattr(store, name) for name in required):
        raise RuntimeError(f"storage metadata missing one of {required}")
    ...
    for idx in torch.nonzero(supported, as_tuple=False).flatten().tolist():
        decoded = decode_storage_physics_pu(
            params[idx].tolist(), masks[idx].tolist(),
            terminal_kv[idx].tolist(), float(base_mva[idx]),
        )
        if decoded is None:
            raise RuntimeError(
                f"supported storage metadata row {idx} is not decodable"
            )
        out[idx] = torch.complex(
            torch.tensor(
                decoded["Ystorage_r_pu"], dtype=torch.float64
            ).reshape(4, 4),
            torch.tensor(
                decoded["Ystorage_i_pu"], dtype=torch.float64
            ).reshape(4, 4),
        )
    if not bool(torch.isfinite(out.real).all() & torch.isfinite(out.imag).all()):
        raise RuntimeError("non-finite storage metadata decode")
