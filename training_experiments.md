...
| T13 fix matrix | 15254842-44, 15255068 | 240f seed0, one var each: PE hopcap 150 / lr 2e-4 / bs16 / fb-points 3 | bs16 FINAL 1.308 FAIL; fb3 DIED ep13 (1.103, failing anyway); pe150/lr2e4 finals pending (1.23 / 0.83 late-run) | The knob is the LOSS (drop I), not PE/bs/fb; lr2e4 partially rescues current-loss-on but does not beat vonly |
| T14 FULL LAUNCH | 15256412 | train_dk_full.sbatch: 4xH100 DDP, ALL feeders, task=random, W_I=0 W_KCL=0 (vonly), warmup, 120 ep | PENDING->RUNNING | fired on the 3-good-runs gate above; per-epoch ckpt to runs/dk_foundation_15256412/last.pt |
| T15 ladder probe | 15256418 | gridfm/probes/ladder_solver_probe.py, 5 feeders x 4 corpora | QUEUED | machine-precision architecture test: ladder(truth Icomp) V err (want 1e-9, held on SMART-DS w/ transformers) + Icomp->V error gain (want O(1) vs V->I 2e7). If both hold: model predicts hidden Icomp, ladder solves V -- precision limited by estimator, not operator |
