...
## Capability matrix (the definition of "foundational" -- user, 2026-07-17)
Reconstruct ANY subset from any other subset, under realistic data quality:
- powerflow (V, I_bus from Y+Icomp+slack): DONE, machine-exact, mesh-capable solve
- state estimation (V from partial/NOISY/BAD measurements): clean case done (joint solve);
  noise + gross-error rejection = NEXT, the NN is non-optional there even when determinate
- injection estimation (Icomp): exact where identifiable; nullspace prior currently ~0 (T21 gap)
- parameter estimation (Y): excitation-limited (T22) -> structured Y head + multi-snapshot samples
- beyond: topology errors, switch states, DER discovery -- discrete, prior-driven
Roadmap order: (1) estimator mechanism beating zero prior; (2) I_bus then P/Q measurement
inputs; (3) noise/bad-data in training masks + weighted solve; (4) structured Y capability.
