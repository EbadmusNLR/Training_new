...
| T14 FULL LAUNCH | 15256412 (s0), 15256576 (s1) | 4xH100 DDP, ALL feeders, task=random, vonly (W_I=0). s0=lr4e-4 120ep/40h; s1=lr2e-4 48ep/16h backfill (combo bet from lr2e4 tie; replaced 15256442) | PENDING (Priority) | fired on the 3-good-runs gate (0.824/0.789/0.848); per-epoch ckpt survives timeouts/shm |
| T15 ladder probe | 15256418, 15256491 (v2: shunt-only perturb, real corpus names) | ladder(truth Icomp) + Icomp->V gain, 5 feeders x 4 corpora | **18/20 converge 1e-8..1e-14, gain ~0, ALL corpora, incl 7.5k-node + transformers**. 2 anomalies: P10U gain ~450 (persists shunt-only); t1_1201_k44_transformer DIVERGES 5.7e+86 (rho>1, suspect magnetizing shunt in the split) | LADDER IS THE MACHINE-PRECISION OPERATOR pending 2 anomaly fixes -> diag 15256636. Design: model estimates hidden SHUNT Icomp (direct supervision, no solver in training loop); ladder solves V as deterministic postprocessor. Next after diag: e2e eval = vonly ckpt ic estimates -> ladder V vs direct V head |
